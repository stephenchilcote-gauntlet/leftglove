...
export function registerVocabularyTools(
  ...
  server.tool(
    ...
    {},
    async () => textResult(JSON.stringify(currentGlossary, null, 2)),
  );
  ...
  server.tool(
    ...
    async () => {
      try {
        ...
        return textResult(`Refreshed: ${n} element(s) across ${currentGlossary.intents.length} intent(s).`);
      } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        return textResult(`Refresh failed: ${msg}`);
      }
    },
  );
}
